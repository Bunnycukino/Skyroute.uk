...
export default function ShipmentLog() {
    ...
    const checkExpiry = async () => {
      ...
      shipments.forEach(shipment => {
        ...
        if (hoursDiff >= 48 && shipment.status !== 'EXPIRED' && shipment.status !== 'COMPLETED') {
          // Jeśli ma dane logistyczne → COMPLETED, jeśli nie → EXPIRED
          // `is_complete` is a custom field for the C208 form to indicate completion
          const hasLogisticData = shipment.logistic_flight_number || shipment.logistic_pieces || shipment.logistic_date || shipment.is_complete;
          const newStatus = hasLogisticData ? 'COMPLETED' : 'EXPIRED';
  ...
  }, [shipments, isAdmin, updateStatusMutation]); // Added updateStatusMutation to dependency array
                ...
                {isLoading ? (
                  ...
                  filteredShipments.map((shipment) => (
                    <TableRow 
                      ...
                      } ${shipment.status === 'COMPLETED' ? 'bg-green-50' : ''} ${selectedIds.includes(shipment.id) ? 'bg-blue-50' : ''}`}
                    >
                      {isAdmin && (
                        <TableCell>
                          <Checkbox 

...
export default function InBondForm() {
  ...
  const { data: shipments = [], isLoading: shipmentsLoading, refetch } = useQuery({
    queryKey: ['shipments'],
  ...
  useEffect(() => {
    if (!shipmentsLoading && shipments.length > 0 && selectedShipmentId === null) {
      setSelectedShipmentId(shipments[0].id);
    }
  }, [shipmentsLoading, shipments, selectedShipmentId]);
  ...
  if (isUserLoading || shipmentsLoading) {
    return (
